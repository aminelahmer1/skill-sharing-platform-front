...
export class ProducerProfileComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private readonly ADDRESS_ZOOM = 14; // Zoom plus rapproché pour mieux voir le marqueur
  private mapInitialized = false;

  // Style moderne pour le marqueur
  private readonly MARKER_STYLE = new Style({
    ...
    text: new Text({
      text: '📍', // Emoji de marqueur
      scale: 1.5,
      offsetY: -20,
      fill: new Fill({ color: '#FF3D00' }) // Rouge-orange
    })
  });
  ...
  private addModernMarker(coords: Coordinate): void {
    const marker = new Feature({
      ...
      name: 'Location' // Propriété utilisée par le style
    });
    ...
    const vectorLayer = new VectorLayer({
      ...
      zIndex: 100 // S'assurer que le marqueur est au-dessus
    });
